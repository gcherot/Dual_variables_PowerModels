...
def _runpm(net, delete_buffer_file=True, pm_file_path=None, pdm_dev_mode=False, **kwargs): 
    ...
    **pdm_dev_mode** (bool, False) - If True, the develop mode of PdM is called.
    ...
    ########################################
    #### Modified code to recover duals ####
    ########################################
    
    # run power models optimization in julia
    results = _call_pandamodels(buffer_file, net._options["julia_file"], pdm_dev_mode)
    net, result_pm = _recover_dual(net, results)
...
def _recover_dual(net, results):
    import pandas as pd
    import numpy as np
    result_pm, dual_cstr_array, dual_cstr_name, dual_var_upr_array, dual_var_lwr_array, dual_var_name = results # Unpack results
    # Create a data frame containing the reuslts
    names = dual_cstr_name + dual_var_name + dual_var_name # Name of constraints 
    lower_bound = len(dual_cstr_name)*[0] + len(dual_var_name)*[1] + len(dual_var_name)*[0] # 1 if the constraint is the lower bound of a JuMP varaible (see PowerModels.model)
    upper_bound = len(dual_cstr_name)*[0] + len(dual_var_name)*[0] + len(dual_var_name)*[1] # 1 if the constraint is the upper bound of a JuMP varaible (see PowerModels.model)
    dual = np.concatenate((dual_cstr_array, dual_var_lwr_array, dual_var_upr_array)) # Value of the dual variable (or lagrage multiplier)
    zipped = list(zip(names, lower_bound, upper_bound, dual)) # Zip list to create DataFrame
    net.dual = pd.DataFrame(zipped, columns=['Name', 'is_lower_bound', 'is_upper_bound', 'dual_value']) # Create dataframe and stores in net.dual
